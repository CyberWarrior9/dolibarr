...
each provided in **Python 3, PHP 8, and shell (bash + curl)**. Every script provisions its own
...
php      <cve-dir>/exploit.php          # PHP 8 CLI + ext-curl (bundled on most installs)
...
attacker keys. `_lab/run-matrix.sh` rebuilds under three hardening profiles and runs all twelve
